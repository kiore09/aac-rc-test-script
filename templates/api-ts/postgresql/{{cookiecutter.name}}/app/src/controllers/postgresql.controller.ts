...
import { createTcpPool } from '../utils/connect-tcp';
...
const createPool = () : Promise<Knex<any, unknown[]>> => {
...
const ensureSchema = async (pool) => {
  const hasTable = await pool.schema.hasTable('contacts');
  if (!hasTable) {
    return pool.schema.createTable('contacts', (table) => {
      table.increments('id').primary();
      table.specificType('first_name', 'CHAR(6)').notNullable();
      table.specificType('last_name', 'CHAR(6)').notNullable();
      table.specificType('email', 'CHAR(6)').notNullable();
      table.timestamp('created_on', Date.now()).notNullable();
    });
  }
  logger.info('Ensured that table \'contacts\' exists');
};
...
const getTableData = async (pool) => {
  return await pool
      .select('first_name', 'last_name', 'email', 'created_on')
      .from('contacts')
      .limit(5);
};
...
const testPostgres = async (req, res) => {
  ...
  pool = pool || createPoolAndEnsureSchema();
  try {
    // Query the 'item' table from the database.
    const data = await getTableData(pool);
    logger.info('Getting data from database...');
    logger.info('data', data);
    res.render('postgresql', {data: data});
  } catch (err) {
...
export { homeView, testPostgres, initializePool };
