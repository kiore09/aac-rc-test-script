...
const projectId: string = config.projectId; // Your Google Cloud Platform project ID
const topicName: string = config.topicName; // Name for the new topic to create Ex: projects/<project_id>>/topics/<topic_name>
const subscriptionName: string = config.subscriptionName; // Name for the new subscription to create Ex: projects/<project_id>>/subscriptions/<subscription_name>
...
const pubSub = (req: Request, res: Response) => {
  res.render('index');
};
 ...
 * @param res Express request object
 */
const tryPubSub = async (req: Request, res: Response) => {
  try {
    // Creates message on a topic
    await publishMessage();

    // Retrieve subscriptions
    const message: string | undefined = await listenForMessages();
...
async function publishMessage(): Promise<void> {
  ...
  try {
    ...
    logger.error(`An error occurred while publishing: ${(err as Error).message}`);
  }
}

async function listenForMessages(): Promise<string | undefined> {
  try {
    ...
    error += `${err.message} <br>`;
    logger.error(
        `An error occurred while retrieving subscriptions: ${(err as Error).message}`,
    );
  }
}

export {pubSub, tryPubSub};
