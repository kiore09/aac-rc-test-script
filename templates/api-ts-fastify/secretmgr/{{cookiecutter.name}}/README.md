...
This is a sample Fastify Typescript application with Google Cloud Secret Manager to demonstrate the following feature implementation:
...
npm run dev
```
By default, `npm run dev` starts Fastify with `--address=0.0.0.0` which suffices for local testing. Use `npm run build` and `npm run start` when deploying the application to a container; this also uses `--address=0.0.0.0` as mentioned in [Fastify's documentation](https://fastify.dev/docs/latest/Reference/Server#server-methods).
