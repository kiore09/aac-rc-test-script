...
[NOTE] - Cloud Functions create their own Subscription for the topic provided as trigger.
...
The following example simulates a Cloud Pub/Sub message from `YOUR_TOPIC` in `YOUR_PROJECT`:
```
  curl localhost:8080 \
  -X POST \
  ...
  -d '{
        "context": {
          ...
          "resource":{
            ...
            "name":"projects/YOUR_PROJECT/topics/YOUR_TOPIC",
        ...
        "data": {
          ...
          "data": "eyJpZCI6MjEsICJmaXJzdE5hbWUiOiJUZXN0aW5nIiwgImxhc3ROYW1lIjoiRGV2ZWxvcGVyIiwgImRhdGVPZkJpcnRoIjoiMDEvMDEvMTkwMCJ9"
        }
      }'
```
The `data` value must be encoded in Base64 - the example `data` above decodes to:
```
  {"id":21, "firstName":"Testing", "lastName":"Developer", "dateOfBirth":"01/01/1900"}
```
