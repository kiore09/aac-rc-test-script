# Data Loss Prevention (Inspection and Classification Solution)
**Using Cloud Function, Storage Buckets and DLP API**

**Event Driven** - A text file is dropped into the source cloud storage bucket, the cloud function automatically picks up the file and classifies the file as either seisitive or non-sensitive by using the DLP API. If the content of the file are classified as containing sensitive data, de-identification process would be performed to mask the sensitive data. After the masking process, the function will then move the file to corresponding storage bucket.

![High level DLP Solution Diagram](docs/dlp-inspect-simple.png)
...
**Data Loss Prevention API** - GCP Cloud Data Loss Prevention API allows you to inspect, analyze, classify, and redact/de-identify sensitive information within data streams in transit, or data stored at rest in structured / relational databases. In this template we integrated the inspection and de-identification service together to mask and move the sensitive data into different storage bucket based on the inspection result. 
You can find additional information about Google Cloud DLP [here](https://cloud.google.com/dlp)

**Cloud Function** - allows you to run your code in the cloud with no servers or containers to manage. You can find additional details about cloud functions [here.](https://cloud.google.com/functions#section-9)
...
- Cloud Data Loss Prevention (DLP) API is enabled
- The source and two destination storage buckets are created
- The default app engine service account has the required roles. see [dlp_infra Terraform](terraform/dlp_infra.tf)
- The dlp activation request has been submitted to the project at least once. see [dlp activateion](docs/dlp_activation.md)
- The viewer role is assigned to the dlp-api service account. see [dlp_srv Terraform](terraform/dlp_srv.tf)
- Configure the Cloud Build Trigger for deployment pipeline.
...
To deploy the function manually, you will only need to create a build trigger for this repository. Cloud Build will then read [cloudbuild.yaml](cloudbuild.yaml) and deploy the function accordingly.

**Before you get started:**
- Make sure your source and target buckets in Cloud Storage are configured correctly.
- Check that the bucket names for `_SOURCE_BUCKET`, `_SENSITIVE_BUCKET` and `_NON_SENSITIVE_BUCKET` are correct in [cloudbuild.yaml](cloudbuild.yaml).
