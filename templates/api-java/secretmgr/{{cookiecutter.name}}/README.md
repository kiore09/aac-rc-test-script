...
This is a sample Google Cloud Secret Manager Application to demonstrate the following feature implementation:  
...
In order to run the GCP Secret Manager sample application, you will need to ensure that the GCP Secret Manager you specify in the cookiecutter.yaml file is created in your GCP Project.
...
The sample application requires a secret with the name `{{cookiecutter.secretmgr.sampleSecret}}` in your GCP project before it can compile properly. [This document](docs/secret-setup.md)
serves as a guide for creating any secrets you might need for the demo.
___
