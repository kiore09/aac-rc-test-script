...
const emailView = (req, res) => {
  ...
  const responseMsg = req.response_msg || '';
  res.render('send', {statuscode: statusCode, response_msg: responseMsg, fromDomain: config.fromDomain});
};
...
const sendEmails = async (req, res, next) => {
  ...
  const clientID = `projects/${config.projectId}/secrets/${config.clientID}/versions/latest`;
  const clientSECRET = `projects/${config.projectId}/secrets/${config.clientSECRET}/versions/latest`;
  const accessTokenURL = config.apigwTokenURL;
  const apiEndpointURL = config.apiEndpointURL;
  const apiScope = config.apigwTokenScope;

  try {
    const versionid = await secretManager.getSecret(clientID);
    const versionsecret = await secretManager.getSecret(clientSECRET);
    logger.info('Retrieving secrets...');
    // Decode client credentials
    const clientIDValue = new TextDecoder('utf-8').decode(versionid.payload.data);
    const clientSECRETValue = new TextDecoder('utf-8').decode(versionsecret.payload.data);
    ...
    const tokenResponse = await axios.post(accessTokenURL, null, tokenOptions);
    const token = tokenResponse.data.access_token;  // Extract token from response
    ...
    const emailResponse = await axios.post(apiEndpointURL, form, emailOptions);
...
module.exports = {homeView, emailView, sendEmails};
